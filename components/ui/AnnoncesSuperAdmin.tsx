...
export default function AnnoncesSuperAdmin() {
  ...
  const load = async () => {
    const [{ data: ann }, { data: v }] = await Promise.all([
      ...
      supabase
        .from("vendors")
        .select("id,boutique_nom,boutique_whatsapp,actif")
        .eq("actif", true),
    ]);
    setAnnonces(ann ?? []);
    setVendors(v ?? []);
  ...
  const sendAnnonce = async (e: React.FormEvent) => {
    ...
    if (sendWA && vendors.length > 0) {
      ...
      vendors.slice(0, 1).forEach((v) => {
        window.open(
          `https://wa.me/${v.boutique_whatsapp}?text=${encoded}`,
          "_blank",
        );
      });
      notify(
        `Annonce envoyée à ${vendors.length} vendeur${vendors.length > 1 ? "s" : ""} !`,
      );
    } else {
  ...
  return (
    <div className="space-y-6 relative">
      ...
      {toast && (
        <div
          className="fixed top-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 px-5 py-3 rounded-full text-sm font-semibold shadow-2xl animate-fade-up"
          style={
            toast.type === "ok"
              ? {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          >
            {vendors.length} vendeur{vendors.length > 1 ? "s" : ""} actif
            {vendors.length > 1 ? "s" : ""} recevront tes messages
          </p>
        </div>
        <button
            ...
            >
              ...
              <div>
                ...
                <p
                  ...
                  style={{ color: "rgba(255,255,255,.35)" }}
                >
                  Le message sera envoyé sur le WhatsApp de chaque vendeur actif
                  ({vendors.length} vendeur{vendors.length > 1 ? "s" : ""}).
                  {!sendWA &&
                    " Désactivé — annonce visible uniquement dans leur admin."}
                </p>
              </div>
            </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    ...
                    style={{ color: "rgba(255,255,255,.4)", lineHeight: 1.6 }}
                  >
                    {ann.contenu}
                  </p>
                  <p
